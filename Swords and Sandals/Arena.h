...
private:
	...
	void handleButtonClick(const std::string& buttonName, Player& player, Enemy& enemy, GameState &gameState);
	void handleButtonClickFightInProgress(const std::string& buttonName, Player& player, Enemy& enemy);
	void handleButtonClickFightEnded(const std::string& buttonName, Player& player, Enemy& enemy, GameState& gameState);
...
public:
	...
	void checkForClickedButton(const sf::Vector2f& mousePosition, Player& player, Enemy &enemy, GameState& gameState);
